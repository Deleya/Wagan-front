...
const AssistantCard: FC<Props> = ({ name, role, image, description, onClick }) => {
  ...
  return (
      <div className="relative w-[16%] h-32  flex flex-col items-center justify-between p-4 transition-transform duration-300 ">
        <div
            onClick={onClick}
            className="rounded-lg bg-black/40  cursor-pointer hover:scale-105 transition-transform group"
        >
          <img src={image} className="w-full h-full object-cover opacity-80 " alt={name} />
